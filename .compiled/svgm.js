 ...
 */
'use strict'; // returns a window with a document and an svg root node
// const window   = require('svgdom');

const SVG = require('svg.js'); // const document = window.document;
...
function prepareSVG(rawSvg, filename) {
  ...
  svgcanvas.setAttribute('hidden', 'true'); // eslint-disable-next-line no-undef
  ...
  GroupItems.forEach(groupitem => {
    ...
    result = result.concat(rows); //progressValue = progressValue + progressStep;
...
function getGroups() {
  ...
  let groups = [];
  if (scalebars) scalebars.forEach(scalebar => {
...
function getItems(groupitem) {
  ...
  let elements = groupitem.group.children(); //debugger;
...
function getLabels(item) {
  // body...
  let attributes = item.attr(); // debugger;
...
function getMeasures(item, idtext, scalefactor) {
  ...
  let arr = idtext.split('-'); //получаем массив символов измерений
  ...
  let chars = matchArr(arr, setchars[itemtype], itemtype); //if (itemtype='g') {debugger;}
  ...
  let measures = {};
  chars.forEach(ch => {
    measures[ch] = measure(item, itemtype + "-" + ch, scalefactor); //measures.push(measure(item, ch));
  }); //debugger;
...
function prepareBudnikovSVG(rawSvg) {
  ...
  draw.svg(rawSvg); //получить группы как родители scalebar
  ...
  let scalefactor = 200 / dist(lineArray.x1, lineArray.x2, lineArray.y1, lineArray.y2);
...
function prepareSecSVG(rawSvg) {
  ...
  let result = [];
  GroupItems.forEach(groupitem => {
    const rows = prepareSecItemGroup(groupitem); //debugger;
...
function prepareSecItemGroup(ItemGroup) {
  ...
  let rows = []; //'PubCode','SectCode','name','FAD','LAD'
